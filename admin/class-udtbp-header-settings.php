...
if ( ! class_exists( 'udtbp_Header_Settings' ) ) :
  class udtbp_Header_Settings extends udtbp_Admin {
    ...
    */
    public function settings_api_init( $items ){
      ...
      */
     $str = sprintf( __('Visibility', UDTBP_NAME) );
      add_settings_field(
        'view-header',
        apply_filters( $this->udtbp . '-view-header', $str ),
      ...
      */
      ...
      add_settings_field(
        'header-title',
        apply_filters( $this->udtbp . '-header-title', $str ),
  ...
  ?>
      ...
      <p><?php echo __( 'Display header with web site title or department text.' ); ?></p>
    <?php
        ...
        <label for="<?php echo esc_attr( $this->udtbp ) ?>_options[view-header]" class="check-switch">
          ...
          <div class="ud-img--container">
            <span class="udt_value_text udt_on_value">
              <svg id="svg_on" class="icon-svg ud-class--on" role="img" aria-label="This icon is displayed when the selected option is turned on." xmlns="http://www.w3.org/2000/svg" focusable="false" width="48" height="48" viewBox="0 0 128 128">
                <defs>
                  <style>
            ...
            <span class="udt_value_text udt_off_value">
              <svg id="svg_off" class="icon-svg ud-class--off" role="img" aria-label="This icon is displayed when the selected option is turned off." xmlns="http://www.w3.org/2000/svg" focusable="false" width="48" height="48" viewBox="0 0 128 128">
                <title id="id-off" class="screen-reader-text">Off icon</title>
    ...
    ?>
      <div id="ud-id-ht" class="box-content">
        <label for="<?php echo esc_attr( $this->udtbp.'_options[header-title]' ) ?>">
          <input id="<?php echo esc_attr( $this->udtbp.'_options[header-title]' ) ?>" name="<?php echo esc_attr( $this->udtbp.'_options[header-title]' ) ?>" type="text" value="<?php echo esc_attr( $option ); ?>">
        </label>
        <input type="hidden" id="udt_custom_header_text" name="udt_custom_header_text" value="<?php echo esc_attr( $option ); ?>">
        <p class="help"><?php __( 'Displays web site Title if no departmental text is provided.' ); ?></p>
      </div>
    <?php
      /**
